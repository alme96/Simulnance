...
class TradingAgent(Agent):
    ...
    def sell_order(self):
        if len(self.model.limit_order_book[0]) == 0:
            ...
            (ask_price, a_deadline, a_id) = min(self.model.limit_order_book[0])  # Ev. problematic to always calculate min/max.
        if len(self.model.limit_order_book[1]) == 0:
            ...
            return
        (bid_price, b_deadline, b_id) = max(self.model.limit_order_book[1])  # maybe easier to have an ordered l_o_b!
        s_order = round(ask_price * (np.random.normal(mean, std, 1)[0]))
        if s_order > bid_price:
            ...
            # do some trading with the current agent and the buyer
            print("Seller ", self.unique_id, " meets Buyer ", buyer.unique_id)
    ...
    def buy_order(self):
        ...
        if b_order < ask_price:
            ...
            # do some trading with the current agent and the seller
            print("Buyer ", self.unique_id, " meets Seller ", seller.unique_id)
